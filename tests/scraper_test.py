...
def mocked_requests_get(*args, **kwargs):
    ...
    if (
        args[0] == "https://www.sreality.cz/hledani/prodej/byty/ostrava,liberec"
        and kwargs["headers"] == Scraper.HEADERS
        and kwargs["params"]["velikost"] == params
    ):
...
class ScraperTest(unittest.TestCase):
    ...
    ostrava_liberec_small = Scraper(
        ["ostrava", "liberec"], ["1+1", "1+kk", "2+kk"], search_type="prodej"
    )
    ostrava_liberec_small_url = (
        "https://www.sreality.cz/hledani/prodej/byty/ostrava,liberec"
    )
    ...
    def test_scraper(self):
        ...
        self.assertEqual(
            self.ostrava_liberec_small._generate_url, self.ostrava_liberec_small_url
        )

    def test_mock_get_page(self):
        with patch(
            "src.scraper.requests_html.HTMLSession.get", side_effect=mocked_requests_get
        ):
            ...
            self.assertEqual(
                properties_render, {"/property1", "/property2", "/property3"}
            )

    def test_mock_async_rendering(self):
        with patch(
            "src.scraper.requests_html.AsyncHTMLSession.get",
            side_effect=mocked_async_requests_get,
        ):
            ...
            future_delay = asyncio.ensure_future(
                self.render_test._run(["/non_ok_reposnse"], timeout=5)
            )
